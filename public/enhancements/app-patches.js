...
(function () {
  ...
  function mountWelcome() {
    ...
    bar.innerHTML =
      ...
      'Welcome back, <strong></strong> — filter corridors in the sidebar, open a row for details, or jump to Multi-Port for recruit opportunities.' +
      '</div>' +
  ...
  function mountHelpBar() {
    ...
    shell.innerHTML =
      ...
      '<span>Click a corridor row for full towage & TNT context</span>' +
      '</div>' +
      '<span>Tug Network Team</span>';
    // Place after app-body so it sits at the bottom of the column layout
    var appRoot = body.parentNode;
    if (appRoot) appRoot.appendChild(shell);
  }

  // Fix layout: help bar should be inside the flex column of body
  function fixHelpBarLayout() {
    var help = document.getElementById('tnt-help-bar');
    var body = document.body;
    if (!help || !body) return;
    // body is flex column with header + app-body; append help as third row
  ...
  function polishTableEmpty() {
    ...
    if (!empty.querySelector('.tnt-empty-cta')) {
